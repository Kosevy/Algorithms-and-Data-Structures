...
private:
    ...
    void cargarCamposFormulario() {
        ...
        // Removido: insertarAlFinal(CampoFormulario<T>("Codigo promocional", "texto", false));
        // Removido: insertarAlFinal(CampoFormulario<T>("Usar millas", "seleccion", false));
    }
...
public:
    ...
    void mostrarMenu() {
        ...
        while (!salir) {
            ...
            Console::SetCursorPosition(15, 5); cout << "2. Buscar vuelos con los criterios del formulario";
            Console::SetCursorPosition(15, 6); cout << "3. Historial de busquedas";
            Console::SetCursorPosition(15, 7); cout << "4. Busqueda por destino";
    ...
    void completarFormulario() {
        ...
        for (int i = 1; i <= totalCampos; i++) {
            ...
            Console::SetCursorPosition(15, 6 + i * 3);
            ...
            if (campoActual.tipo == "seleccion") {
                if (campoActual.nombreCampo == "Clase de cabina") {
                    cout << "\n";
                    Console::SetCursorPosition(20, 7 + i * 3);
                    cout << "1. Economy  2. Business  3. First Class";
                    Console::SetCursorPosition(20, 8 + i * 3);
                    cout << "Seleccione (1-3): ";
                    int opcionCabina;
                    cin >> opcionCabina;
                    switch (opcionCabina) {
        ...
        for (size_t i = 0; i < formulario.campos.size() && i < 6; i++) {
    ...
    void buscarVuelosConCriterios() {
        Console::Clear();
        Console::ForegroundColor = ConsoleColor::Yellow;
        Console::SetCursorPosition(35, 2); cout << "=== BUSCAR VUELOS CON CRITERIOS ===";
        Console::ForegroundColor = ConsoleColor::White;

        if (pilaFormularios.estaVacia()) {
            mostrarMensaje("SIN CRITERIOS", "Primero debe completar el formulario de busqueda (opcion 1).", ConsoleColor::Red);
            return;
        }

        // Crear una copia temporal para acceder al último formulario sin modificar la pila original
        PilaPilasColas<FormularioCompleto<T>> pilaTemporal;
        FormularioCompleto<T> ultimoFormulario;
        bool formularioEncontrado = false;

        // Vaciar la pila original y guardar en temporal
        while (!pilaFormularios.estaVacia()) {
            FormularioCompleto<T> form = pilaFormularios.pop();
            pilaTemporal.push(form);
        ...
        while (!pilaTemporal.estaVacia()) {
            pilaFormularios.push(pilaTemporal.pop());
        }

        if (!formularioEncontrado) {
            mostrarMensaje("ERROR", "No se pudo acceder al formulario.", ConsoleColor::Red);
            return;
        }

        // Extraer criterios del formulario
        string origen = "", destino = "", fecha = "";
        int pasajeros = 1;
        string cabina = "Economy";

        for (const auto& campo : ultimoFormulario.campos) {
            if (campo.nombreCampo == "Origen") origen = campo.valor;
            if (campo.nombreCampo == "Destino") destino = campo.valor;
            if (campo.nombreCampo == "Fecha de ida") fecha = campo.valor;
            if (campo.nombreCampo == "Numero de pasajeros") pasajeros = stoi(campo.valor);
            if (campo.nombreCampo == "Clase de cabina") cabina = campo.valor;
        }

        Console::SetCursorPosition(15, 4); cout << "Buscando vuelos con los siguientes criterios:";
        Console::SetCursorPosition(15, 5); cout << "--------------------------------------------";
        Console::SetCursorPosition(15, 6); cout << "Origen: " << origen;
        Console::SetCursorPosition(15, 7); cout << "Destino: " << destino;
        Console::SetCursorPosition(15, 8); cout << "Fecha: " << fecha;
        Console::SetCursorPosition(15, 9); cout << "Pasajeros: " << pasajeros;
        Console::SetCursorPosition(15, 10); cout << "Cabina: " << cabina;
        Console::SetCursorPosition(15, 11); cout << "--------------------------------------------";

        // Simular búsqueda de vuelos
        Console::SetCursorPosition(15, 13); cout << "Buscando vuelos disponibles...";
        Console::SetCursorPosition(15, 15); cout << "Resultados encontrados:";
        Console::SetCursorPosition(15, 16); cout << "1. " << origen << " -> " << destino << " - LATAM - $350 - " << fecha;
        Console::SetCursorPosition(15, 17); cout << "2. " << origen << " -> " << destino << " - Avianca - $420 - " << fecha;
        Console::SetCursorPosition(15, 18); cout << "3. " << origen << " -> " << destino << " - Sky Airline - $280 - " << fecha;

        Console::SetCursorPosition(15, 20); cout << "Para ver mas opciones, vaya al catalogo de vuelos disponibles";
    ...
    void mostrarFormularioActual() {
        ...
        Console::SetCursorPosition(15, 13); cout << "Presione cualquier tecla para continuar...";
        _getch();
    }

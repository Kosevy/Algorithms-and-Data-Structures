...
private:
    ...
    void insertar(NodoReserva<T>*& nodo, const T& vuelo) {
        if (!nodo) {
            nodo = new NodoReserva<T>(vuelo);
        }
        else if (vuelo.fecha < nodo->vuelo.fecha) {
            insertar(nodo->izquierda, vuelo);
        }
        else {
            insertar(nodo->derecha, vuelo);
        }
    }
    ...
    void eliminarReservaEspecifica(int totalReservas) {
        ...
        // Mostrar detalles de la reserva a eliminar
        ...
        if (confirmacion == 'S' || confirmacion == 's') {
            if (eliminarReserva(numReserva)) {
                ...
                // Verificar si habia hotel asociado
    ...
    // Funcion auxiliar para eliminar todo el arbol
...
public:
    ...
    // Funcion para obtener detalles de una reserva especifica
    ...
    // Menu completo para modificar reservas
    ...
    // Menu completo para eliminar reservas

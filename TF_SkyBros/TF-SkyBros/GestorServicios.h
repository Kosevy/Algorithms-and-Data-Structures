...
private:
    ...
    NodoAVLHotel* insertar(NodoAVLHotel* nodo, const Hotel& hotel) {
        if (!nodo) return new NodoAVLHotel(hotel);                  // O(1): comprobación y creación de nodo
        if (hotel < nodo->hotel)                                    // O(1): comparación
            nodo->izq = insertar(nodo->izq, hotel);                // O(log n): llamada recursiva en subárbol izquierdo
        else if (hotel > nodo->hotel)                               // O(1): comparación
            nodo->der = insertar(nodo->der, hotel);                // O(log n): llamada recursiva en subárbol derecho
        else
            return nodo;                                            // O(1): retorno inmediato
        actualizarAltura(nodo);                                     // O(1): actualización de altura
        int bal = balance(nodo);                                    // O(1): cálculo de factor de balanceo
        if (bal > 1 && hotel < nodo->izq->hotel)                    // O(1): comprobación y comparación
            return rotarDerecha(nodo);                              // O(1): rotación simple
        if (bal < -1 && hotel > nodo->der->hotel)                   // O(1): comprobación y comparación
            return rotarIzquierda(nodo);                            // O(1): rotación simple
        if (bal > 1 && hotel > nodo->izq->hotel) {                  // O(1): comprobación y comparación
            nodo->izq = rotarIzquierda(nodo->izq);                  // O(1): rotación doble (parte 1)
            return rotarDerecha(nodo);                              // O(1): rotación doble (parte 2)
        }
        if (bal < -1 && hotel < nodo->der->hotel) {                 // O(1): comprobación y comparación
            nodo->der = rotarDerecha(nodo->der);                    // O(1): rotación doble (parte 1)
            return rotarIzquierda(nodo);                            // O(1): rotación doble (parte 2)
        }
        return nodo;                                                // O(1): retorno
    }
...
public:
    ...
    string reservarHotel(const string& ciudadDestino) {
        if (reservaHotelPorDestino[ciudadDestino]) {
            ...
            cout << "No se puede reservar mas de 1 hotel por destino.";

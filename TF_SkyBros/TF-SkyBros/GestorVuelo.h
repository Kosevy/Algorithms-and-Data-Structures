...
class GestorVuelos {
private:
    ArbolReservas<Vuelo<T>>* arbolReservas;
    vector<Vuelo<T>> vuelos;
    AVLVuelosPublicos arbolVuelosPublicos;

    void cargarVuelosAleatorios(int cantidad) {
        vuelos.clear(); // Limpiar los vuelos existentes
        arbolVuelosPublicos = AVLVuelosPublicos(); // Limpiar el arbol AVL
        for (int i = 0; i < cantidad; ++i) {
            vuelos.push_back(generarVuelo<T>()); // Generar y agregar vuelos aleatorios
        }
        for (const auto& v : vuelos) {
            VueloPublico vp(v.origen, v.destino, v.aerolinea, v.precio, v.fecha, v.hora);
            arbolVuelosPublicos.agregarVuelo(vp);
        }
    }
...
public:
    GestorVuelos(ArbolReservas<Vuelo<T>>* reservas) : arbolReservas(reservas) {
    ...
    void mostrarMenu() {
        ...
        while (!salir) {
            ...
            Console::SetCursorPosition(15, 6); cout << "3. Filtrar por aerolinea"; // Busqueda lineal - ver si se puede reemplazar por Busqueda binaria
    ...
    // Filtro Aerolinea - Busqueda lineal
    ...
    void reservarVuelo() {
        ...
        int indice;
        Console::SetCursorPosition(15, 36); // Ajustar posicion para evitar solapamiento
        cout << "Ingrese el numero del vuelo que desea reservar: ";
        cin >> indice;

        if (indice < 1 || indice > vuelos.size()) {
            Console::SetCursorPosition(15, 37);
            cout << "Numero de vuelo invalido. Presione cualquier tecla para continuar...";
            _getch();
            return;
        }
        ...
        if (!vueloSeleccionado.disponible) {
            Console::SetCursorPosition(15, 37);
            cout << "El vuelo seleccionado no esta disponible. Presione cualquier tecla para continuar...";
            _getch();
            return;
        }
        ...
        if (arbolReservas) {
            arbolReservas->agregarReserva(vueloSeleccionado);
        }
        Console::SetCursorPosition(15, 37);
    ...
    void mostrarVuelosPublicos() {
        arbolVuelosPublicos.mostrarVuelos();
    }
...
private:
    ...
    int particion(vector<Vuelo<T>>& vuelos, int inicio, int fin) {
        double pivote = vuelos[fin].precio;           // O(1): acceso y asignación
        int i = inicio - 1;                           // O(1): asignación

        for (int j = inicio; j <= fin - 1; j++) {     // O(n): ciclo principal
            if (vuelos[j].precio <= pivote) {         // O(1): comparación
                i++;                                  // O(1): incremento
                swap(vuelos[i], vuelos[j]);           // O(1): intercambio de elementos
            }
        }

        swap(vuelos[i + 1], vuelos[fin]);             // O(1): intercambio final
        return i + 1;                                 // O(1): retorno
    }

    void ordenarQuickSort(vector<Vuelo<T>>& vuelos, int inicio, int fin) {
        if (inicio < fin) {                                           // O(1): comparación
            int indicePivote = particion(vuelos, inicio, fin);        // O(n): llamada partición
            ordenarQuickSort(vuelos, inicio, indicePivote - 1);       // T(n/2): llamada recursiva sobre subarreglo izquierdo
            ordenarQuickSort(vuelos, indicePivote + 1, fin);          // T(n/2): llamada recursiva sobre subarreglo derecho
        }
    }
    ...
    void ordenarMergeSort(vector<Vuelo<T>>& vuelos, int inicio, int fin) {
        if (inicio < fin) {                                   // O(1): comparación
            int medio = inicio + (fin - inicio) / 2;          // O(1): cálculo del punto medio
            ordenarMergeSort(vuelos, inicio, medio);          // O(log n): llamada recursiva sobre la mitad izquierda
            ordenarMergeSort(vuelos, medio + 1, fin);         // O(log n): llamada recursiva sobre la mitad derecha
            merge(vuelos, inicio, medio, fin);                // O(n): combinación de los subarreglos
        }
    }

...
public:
    ...
    vector<int> dijkstra(int inicio, vector<int>& predecesores) {
        vector<int> distancia(numNodos, numeric_limits<int>::max());      // O(V): inicialización de distancias
        predecesores.assign(numNodos, -1);                                // O(V): inicialización de predecesores

        distancia[inicio] = 0;                                            // O(1): asignación
        priority_queue<pair<int, int>, vector<pair<int, int>>, greater<>> cola; // O(1): creación de cola de prioridad
        cola.push({ 0, inicio });                                         // O(log V): inserción en la cola

        while (!cola.empty()) {                                           // O(E log V): recorrido total
            int distActual = cola.top().first;                            // O(1): acceso al tope de la cola
            int nodoActual = cola.top().second;                           // O(1): acceso al tope de la cola
            cola.pop();                                                   // O(log V): extracción de la cola

            if (distActual > distancia[nodoActual]) continue;             // O(1): comparación

            for (auto& vecino : adyacencia[nodoActual]) {                 // O(grado(nodoActual)): recorrido de vecinos
                int nodoVecino = vecino.first;                            // O(1): acceso a vecino
                int peso = vecino.second;                                 // O(1): acceso a peso

                if (distancia[nodoVecino] > distActual + peso) {          // O(1): comparación y suma
                    distancia[nodoVecino] = distActual + peso;            // O(1): actualización de distancia
                    predecesores[nodoVecino] = nodoActual;                // O(1): actualización de predecesor
                    cola.push({ distancia[nodoVecino], nodoVecino });     // O(log V): inserción en la cola
                }
            }
        }

        return distancia;                                                 // O(1): retorno del vector
    }
